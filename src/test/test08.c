    ...
    )
{
    ...
    http_request_body.base = "name=test08_";
    ...
    )
{
    ...
    if ( is_second_visit) {
        ...
        TEST_EQ( uvhttp_client_request( client, "https://127.0.0.1:8011/test08_", "GET", "User-Agent: UVHttp\r\nContent-Length: 12\r\n", 0) == UVHTTP_OK);
        is_second_visit = 1;
    }
}
    ...
    )
{
    ...
    if ( strcmp( request->uri, "/test08_") == 0) {
    ...
    )
{
    ...
    if ( strcmp( request->uri, "/test08") == 0) {
    }
    else if ( strcmp( request->uri, "/test08_") == 0) {
    }
    else {
        TEST_EQ( 0);
    }
    ...
    )
{
    ...
    if ( strcmp( request->uri, "/test08") == 0) {
        TEST_EQ( strcmp( request_body, "name=test08") == 0);
    }
    else if ( strcmp( request->uri, "/test08_") == 0) {
        TEST_EQ( strcmp( request_body, "name=test08_") == 0);
    }
    else {
        TEST_EQ( 0);
    }
...
void do_test08(){
    printf( "TEST: uvhttp_client test begin\n");
    {
        ...
        http_request_body.base = "name=test08";
        http_request_body.len = strlen( http_request_body.base);
        TEST_EQ( uvhttp_client_request( client, "https://127.0.0.1:8011/test08", "GET", "User-Agent: UVHttp\r\n", &http_request_body) == UVHTTP_OK);
        uvhttp_run( loop);
