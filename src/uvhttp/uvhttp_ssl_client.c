...
static void ssl_connect_user_call( 
...
{
    struct uvhttp_ssl_client* ssl = (struct uvhttp_ssl_client*)stream;
    if ( nread <= 0) {
        goto cleanup;
    }
    if ( ssl->is_closing) {
...
{
    struct uvhttp_ssl_client* ssl = (struct uvhttp_ssl_client*)ctx;
...
{
    int ret = 0;
    struct uvhttp_ssl_client* ssl = (struct uvhttp_ssl_client*)req->data;
    if( status != 0) {
    ...
    )
{
    ...
    ret = uv_read_start( (uv_stream_t*)ssl, uvhttp_ssl_alloc_cb, uvhttp_ssl_read_cb);
    if ( ret != 0) {
        goto cleanup;
    }
    while ( (ret = mbedtls_ssl_handshake_step( &ssl->ssl )) == 0) {
